...
//output directory
#define OUTPUT_DIRECTORY "/home/phil/Documents/courses/"
...
void generateCourse(int numberOfPlanes, int numberOfWayPoints, int fieldLength, int courseNumber)
{
	...
	char filename[1000];
	//sprintf(filename, "%s/%s%d_%dm_%d.course", OUTPUT_DIRECTORY,fileName, numberOfPlanes, fieldLength, courseNumber);
	...
	char baseName[1000];
	sprintf(baseName, "%s%d_%dm_%d.course",fileName, numberOfPlanes, fieldLength, courseNumber);
...
int main()
{
	...
	std::string autoFile;
	std::cout << "Enter the name of the automator file you want to generate: ";
	std::getline(std::cin,autoFile);

	char autoPathName [1000];
	//sprintf(autoPathName,"%s/%s%s",OUTPUT_DIRECTORY,autoFile.c_str(),AUTO_EXTENSION);
