...
{    
	...
	double time = ros::Time::now().toSec();//phil
	...
	*/
	if (findDistance(msg->currentLatitude, msg->currentLongitude, 
					...
					requestWaypointInfoSrv.response.longitude) < WAYPOINT_THRESHOLD){
...
//phil
 ...
 if(!timeTested){
	...
	if((ros::Time::now().toSec()-startTime  > 500)  &&  (ros::Time::now().toSec()-startTime  < 700)){
		ROS_ERROR("HERE");
		timeTested = true;
		FILE* ffpp = fopen("/home/phil/telTimesAPF.txt","w");
...
void formFlocks(void) {
	...
	for (int i = 0; i < (int)pobjects.size(); i++){
		...
		pobjects[i].getLatitude(), pobjects[i].getLongitude()) < 2*WAYPOINT_THRESHOLD) {
			deflock(i);
		}
