...
#define SIMULATION_TIME 600//00

#define CHECK_LENGTH 30//0  //how often we check to see if evaluator is done(every 30 sec)
...
#define DONE_FILE_FULL "/home/phil/fuerte_workspace/sandbox/AU_UAV_stack/AU_UAV_ROS/courses/doneWithCourse.txt"

#define INPUT_DIRECTORY "/home/phil/fuerte_workspace/sandbox/AU_UAV_stack/AU_UAV_ROS/courses/"
#define SCORES_DIRECTORY "/home/phil/fuerte_workspace/sandbox/AU_UAV_stack/AU_UAV_ROS/scores/"
...
/* ************ FIRST RUN THE EVALUATOR ON ALL FILES ************** */
	...
	int defaultSleep = 30;//how long we sleep after launching evaluation, no matter what
	...
	//some bs just for me, feel free to delete
/*
	switch(filename[0]){
		case 'a': 
			defaultSleep = 3;
	...
	if(fp != NULL)
	{
		...
		{	
			...
			if(pid == 0)
			{
				...
				//child process
				system("roslaunch AU_UAV_Team1 evaluationAPF.launch");
...
/* ******************  NOW THE SCORE SHEET ANALYSIS ******************* */
	...
	/* now we average everything out and write it out to a file*/
			...
			fprintf(dataFP,"Average Conflicts: %f\n"
					...
					"Average effiency: %f\n",
			           avgConflicts,avgCollisions,avgActualDist,avgMinDist,effiency);
