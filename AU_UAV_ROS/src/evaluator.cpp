...
std::map<int, std::list<AU_UAV_ROS::waypoint> > waypointLists; //phil
//AU_UAV_ROS::waypoint lastAvoidWP = {0,0,0};
...
//gets the minimum distance traveled for plane
/*double getMinDist(int id){
	double dist = 0;
	double lat;
	double long1;

	std::list<AU_UAV_ROS::waypoint>::iterator it;
	std::list<AU_UAV_ROS::waypoint>::iterator prev;;//=waypointLists[id].begin();
	for(it=waypointLists[id].begin(); it != waypointLists[id].end(); ){//go through all normal waypoints
		//it--;
		//lat = it->latitude;
		//long1 = it->longitude;
		//it++;
		//dist+= findDistance(lat,long1,it->latitude,it->longitude);

		
		dist+= distanceBetween(it,it++);
		it;	
	}//for
...
void endEvaluation()
{
	...
	else
	{
		...
		for(int id = 0; id <= lastPlaneID; id++)
		{
			...
			//fprintf(fp, "%d\t\t%lf\t\t%lf\t\t\t%d\t\t\t", id, waypointDistTraveled[id], getMinDist(id), waypointsAchieved[id]);
			if(isDead[id])
			{
				fprintf(fp, "%lf\n", timeOfDeath[id].toSec());
			}
	...
	if(fp != NULL)
	{
		...
		while(fgets(buffer, sizeof(buffer), fp))
		{
			...
			else
			{
				...
				waypointLists[planeID].push_back(temp);//phil
				...
				if(isFirstPoint[planeID]) {
					...
					lastAvoidWaypoints[planeID] = {0,0,0};
				}
			}
		}//while
