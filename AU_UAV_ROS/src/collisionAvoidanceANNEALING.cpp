...
#include "AU_UAV_ROS/StartCollisionAvoidance.h" //phil
...
const double FIELD_LONGITUDE = -85.490356;
...
/*
	...
	if((ros::Time::now().toSec()-startTime  > 500)  &&  (ros::Time::now().toSec()-startTime  < 700)){
		ROS_ERROR("HERE");
		timeTested = true;
		FILE* ffpp = fopen("/home/phil/telTimesCONES.txt","w");
...
//to end the busy waiting in the main method
...
bool loopCheck(int planeID, int requiredDist){
  ...
  planeBearing = information[planeID][PLANE_BEARING];
  x = information[planeID][WAYPOINT_X] - information[planeID][PLANE_X];
  y = information[planeID][WAYPOINT_Y] - information[planeID][PLANE_Y];

  distanceToTarget = sqrt(x * x + y * y);
  //targetBearing = information[i][WAYPOINT_BEARING];
  targetBearing = information[planeID][WAYPOINT_BEARING];
