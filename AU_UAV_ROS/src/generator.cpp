...
//output directory
#define OUTPUT_DIRECTORY "/home/phil/fuerte_workspace/sandbox/AU_UAV_stack/AU_UAV_ROS/courses/"
...
int main()
{
	...
	//these hold one value for each course
	...
	//these will potentially hold values that are common for all courses
	//if they exists
	int nPlanes = -1;		
	int nWayPoints = -1;
	int nFields = -1;
	...
	if(prompt && needOne("planes",&nPlanes)){}//do nothing else
		...
		std::cout << "Enter number of planes for each course separated by a space:\n\n ";
	...
	if(prompt && needOne("wayPoints",&nWayPoints)){}
		...
		std::cout << "Enter number of wayPoints for each plane in each course separated by a space:\n ";
		std::cout << "(The number of wayPoints in a given course is constant fo every plane.)\n";
		while(count<numSims-1){
	...
	if(prompt && needOne("field length",&nFields)){}
		...
		std::cout << "Enter length of the field for each course separated by a space:\n\n ";
		while(count<numSims-1){
