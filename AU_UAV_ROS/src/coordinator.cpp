		...
		if(planesArray[req.planeID].goToPoint(pointFromService, req.isAvoidanceManeuver, req.isNewQueue))
		{
			...
			*/
			if(req.isNewQueue)
			{
				...
				ROS_ERROR("NOT NEW QUEUE, NO COMMAND SENT");//not error, just a print statement
			}
...
void telemetryCallback(const AU_UAV_ROS::TelemetryUpdate::ConstPtr& msg)
{
	...
	{
		//ROS_ERROR("COORD TEL CAL  LAT: %f",msg->currentLatitude);
...
bool clearCoordinator(AU_UAV_ROS::ClearCoordinator::Request &req, AU_UAV_ROS::ClearCoordinator::Response &res){
	ROS_ERROR("COORD: %s",req.hello.c_str());
	numPlanes = 0;
	planesArray.clear();
}
...
int main(int argc, char **argv)
{
	...
	ros::ServiceServer clearCoordinatorServer = n.advertiseService("clear_coordinator",clearCoordinator);
