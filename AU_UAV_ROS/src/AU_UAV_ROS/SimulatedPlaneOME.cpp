...
//ROS headers
#include "AU_UAV_ROS/standardDefs.h"	//MPS_SPEED, TELEMETRY_FREQUENCY
...
AU_UAV_ROS::SimulatedPlane::SimulatedPlane(long long int planeID, AU_UAV_ROS::CreateSimulatedPlane::Request &requestFromUser)
{
	...
	//printf("SIM CONST: L: %f\n",requestFromUser.startingLatitude);
...
bool AU_UAV_ROS::SimulatedPlane::fillTelemetryUpdate(AU_UAV_ROS::TelemetryUpdate *tUpdate)
{
	...
	/*
		...
		*/
		...
		//ROS_ERROR("LAT1: %f   CUR: %f   LtR: %f   StL: %f",lat1,this->currentLocation.latitude,lat1*RADIANS_TO_DEGREES,(MPS_SPEED*cos(this->actualBearing*DEGREES_TO_RADIANS))*METERS_TO_LATITUDE);
		...
		//double temp = pow(sin((VELOCITY/EARTH_RADIUS)/2.0), 2);
