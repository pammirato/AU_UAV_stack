...
bool AU_UAV_ROS::PlaneCoordinator::handleNewUpdate(AU_UAV_ROS::TelemetryUpdate update, AU_UAV_ROS::Command *newCommand)
{
	...
	if(update.planeID==0){
		//ROS_ERROR("PCOORD:  PLAT: %f   PLONG: %f   DLAT: %f  DLAT: %f",current.latitude,current.longitude,planeDest.latitude,planeDest.longitude);
	}
	...
	if(!avoidancePath.empty() && update.distanceToDestination > -1 && update.distanceToDestination < WAYPOINT_THRESHOLD)//30)
	{
	...
	else if(!normalPath.empty() && update.distanceToDestination > -1 && update.distanceToDestination < WAYPOINT_THRESHOLD)//30)// COLLISION_THRESHOLD)
	{
		// now check if planeDest has been updated
